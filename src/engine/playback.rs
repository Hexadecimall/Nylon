...
pub struct PlaybackEngine {
    ...
    // Scratch the mixer sums into. Owned so the render path borrows it
...
impl PlaybackEngine {
    ...
    ///
    /// There are no sound sources yet, so every track contributes silence.
    /// The transport advances, the meters run, and `events` still land on
    /// the sample they name.
    pub fn render_block(&mut self, output: &mut [[f32; 2]], events: &[MixEvent]) {
        self.take_settings();
        // No sources yet: the mixer is handed no track input, which it
        // treats as silence while still advancing every strip.
        if self.mixer.render(&[], output, events).is_err() {
            // A block the mixer refuses must still be silent rather than
            // whatever the device left in the buffer.
            output.fill([0.0, 0.0]);
        }
        self.transport.advance(output.len());
        self.report();
    }
}
