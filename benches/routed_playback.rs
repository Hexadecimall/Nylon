//! Full callback cost for a routed audio arrangement.
...
fn main() {
    ...
    let mut graph = RoutingGraph::new(TRACKS + 1).unwrap();
    for track in 0..TRACKS {
        graph
            .add_edge(Edge {
                source: track as u16,
                destination: TRACKS as u16,
                kind: EdgeKind::Main,
                gain: 1.0,
            })
            .unwrap();
    ...
    assert!(
        publisher
            .publish_routing(&graph.compile().unwrap(), TRACKS as u16)
            .unwrap()
    );
    ...
    println!("routed_playback_ns_per_block={elapsed:.2}");
    let budget = FRAMES as f64 / f64::from(RATE) * 1.0e9;
    println!(
        "routed_playback_percent_of_budget={:.3}",
