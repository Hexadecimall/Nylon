...
QString deviceKindName(nylon::DeviceKind kind)
{
    ...
    case nylon::DeviceKind::Phaser: return "phaser";
    }
    return "utility";
}
    ...
    const QStringList& positional, int kindIndex, nylon::TrackDevice& device)
{
    ...
    if (kind == "reverb" && positional.size() == kindIndex + 8) {
        device.kind = nylon::DeviceKind::Reverb;
    ...
    if (kind == "phaser" && positional.size() == kindIndex + 8) {
        device.kind = nylon::DeviceKind::Phaser;
        for (int index = 0; index < 7; ++index) {
            if (!parameter(positional[kindIndex + 1 + index], device.parameters[index]))
                return false;
        }
...
QJsonObject deviceJson(const nylon::TrackDevice& device, std::uint64_t index)
{
    ...
    case nylon::DeviceKind::Chorus:
        ...
        break;
    case nylon::DeviceKind::Reverb:
        parameters = {{"size", device.parameters[0]},
            {"decaySeconds", device.parameters[1]}, {"damping", device.parameters[2]},
            {"diffusion", device.parameters[3]},
    ...
    case nylon::DeviceKind::Phaser:
        parameters = {{"rateHz", device.parameters[0]},
            {"centerHz", device.parameters[1]}, {"depthOctaves", device.parameters[2]},
            {"feedback", device.parameters[3]}, {"mix", device.parameters[4]},
            {"stereoPhase", device.parameters[5]}, {"stages", device.parameters[6]}};
        break;
    }
